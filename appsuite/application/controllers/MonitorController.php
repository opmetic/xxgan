...
class MonitorController extends Zend_Controller_Action_Qn
{    
    ...
    public function showstatusAction()
    {
        ...
        else if ($user['workid'] == '88114998')
        {
            $view->assign('skill', 9);
            $view->assign('pno1', 97);
            $view->assign('pno2', 107);  
            $view->assign('flag', 'sc');
        }
        else if ($user['workid'] == '88214999')
        {
            $view->assign('skill', 9);
            $view->assign('pno1', 111);
            $view->assign('pno2', 112);  
            $view->assign('flag', 'sc');  
        }
        else if ($user['workid'] == '88214998')
        {
            $view->assign('skill', 9);
            $view->assign('pno1', 115);
            $view->assign('pno2', 116);  
            $view->assign('flag', 'sc');  
        }
        else if ($user['workid'] == '88116999')
        {
            $view->assign('skill', 8);
            $view->assign('pno1', 98);
        ...
        else if ($user['workid'] == '88116998')
        {
            $view->assign('skill', 8);
            $view->assign('pno1', 123);
            $view->assign('pno2', 124);  
            $view->assign('flag', 'gz');  
        }
        else if ($user['workid'] == '88216999')
        {
            $view->assign('skill', 8);
            $view->assign('pno1', 113); 
            $view->assign('pno2', 114);
            $view->assign('flag', 'gz');   
        }
        else if ($user['workid'] == '88114997')
        {
            $view->assign('skill', 9);
            $view->assign('pno1', 117);
            $view->assign('pno2', 118);  
            $view->assign('flag', 'sc');
        }
        else if ($user['workid'] == '88116997')
        {
            $view->assign('skill', 8);
            $view->assign('pno1', 119);
            $view->assign('pno2', 120);  
            $view->assign('flag', 'gz');
        }
        else if ($user['workid'] == '88136999')
        {
            $view->assign('skill', 8);
            $view->assign('pno1', 121);
            $view->assign('pno2', 122);  
            $view->assign('flag', 'gz');
        }
        
        $view->display('monitorshowstatusEx.tpl');     
    }
    
    public function showstatusexnodeAction()
    {
        $view = Zend_Registry::get('view');
        $user = Zend_Registry::get('user');
        $db = Zend_Registry::get('dbAdapter');
        ...
        $view->display('monitorshowstatusExNode.tpl');
    }
    ...
    public function agentstatusAction()
    {
        ...
        //加载用户姓名
        $select = $db->select();
        $select->from($config['factory_enterprisecode'] . '_' . TABLE_QNSOFT_USER, array('workid', 'nickname'));
        $select->where('deleteflag = 0'); //未删除 
        $listData = $db->fetchAll($select);

        $json = Zend_Json::encode($listData);
        $view->assign('namesjson', $json);
        
        $view->display('monitoragentstatus.tpl');     
    }
    
    public function agentstatusheadAction()
    {
        $view = Zend_Registry::get('view');
        $user = Zend_Registry::get('user');
        $db = Zend_Registry::get('dbAdapter');
        ...
        $view->display('monitoragentstatushead.tpl');     
    }
    
    public function agentstatustableAction()
    {
        $view = Zend_Registry::get('view');
        $user = Zend_Registry::get('user');
        $db = Zend_Registry::get('dbAdapter');
        ...
        $count = array();
        for($i = 1; $i < 600; $i++) $count[] = $i;
        $view->assign('count', $count);
        $view->display('monitoragentstatustable.tpl');     
    }
