...
class QnauthController extends Zend_Controller_Action_Qn
{    
	...
	public function editAction()
	{
		...
		else
		{
            ...
            $db->update(TABLE_QNSOFT_AUTH, $up_ary, "role_id = " . $roleData['role_id']);

            foreach($resourcelist as $node)
            {
                //判断关系是否存在
                $select = $db->select();
                $select->from(TABLE_QNSOFT_AUTH, 'role_id');
                ...
                if ($temp)
                {
                    ...
                    $db->update(TABLE_QNSOFT_AUTH, $up_ary, "role_id = " . $roleData['role_id'] . " and resource_id = " . $node);
                }
                else
                {
                    //新增
                    ...
                    $db->insert(TABLE_QNSOFT_AUTH, $in_ary); 
                }
            }

            $urlArray = array(
                array('title' => "查看列表", 'url' => "/qnrole/list")
            );
