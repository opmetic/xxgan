...
class QnbuttonrenterController extends Zend_Controller_Action_Qn
{    
    ...
    */
    public function listaAction()
    {
        ...
        $select = $db->select();
        $select->from($config['factory_enterprisecode'] .'_' . TABLE_QNSOFT_BUTTON, 'count(*)');
        ...
        $select = $db->select();
        $select->from($config['factory_enterprisecode'] . '_' . TABLE_QNSOFT_BUTTON . ' as p', '*');
        $select->where('button_group >= 100');
        $select->where('button_group <= 101');
        ...
        //分页 
        $page_arr = array(
            'url_start' => $config['site_url'] . '/qnbuttonrenter/lista/',
            'cur_page' => $page, 
        ...
        $view->display('renter/buttonlista.tpl');
    }
    ...
    public function addAction()
    {
    	...
    	$view->display('renter/buttonadd.tpl');
    }
	...
	public function adddoAction()
    {
		...
		if ($button_name && $button_info)
		{
			$in_ary = array(
                ...
                $this->msg("新增按键成功", "/qnbuttonrenter/add"); 
		}
        else
        {
            $this->msg("新增按键失败，值不能为空", "/qnbuttonrenter/add");    
        }
	...
	public function editAction()
	{
		...
		if (!$submit)
		{
			...
			$view->display('renter/buttonedit.tpl');
		}
		else
		{
		    $button_name = $this->_getParam('button_name');
            ...
            $urlArray = array(
                array('title' => "查看列表", 'url' => "/qnbuttonrenter/lista")
            );
	...
	public function delAction()
	{
		...
		$where = $db->quoteInto('button_id = ?', $id);
		$db->update($config['factory_enterprisecode'] . '_' . TABLE_QNSOFT_BUTTON, $up_ary, $where);
		header('location: ' . $config['site_url'] . '/qnbuttonrenter/lista');
	}
    /**
     * 
     */
