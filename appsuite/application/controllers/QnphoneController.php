...
{    
	...
	public function adddoAction()
    {
		...
		$basemodel = $this->_getParam('basemodel');
		if ($phone_num)
		{
			$in_ary = array(
/*					'plain_pwd' => $plain_pwd,
...
*/
	...
	public function listAction()
	{
        ...
        $phone_num = $this->_getParam('phone_num', "");//
        $machineip = $this->_getParam('machineip', "");//
        
        $select = $db->select();
        $select->from(TABLE_QNSOFT_FACTORY, '*');
        $select->where('deleteflag = 0'); //未删除 
        $factoryList = $db->fetchAll($select);
        $view->assign('factoryList', $factoryList);
        
        $select = $db->select();
        $select->from(TABLE_QNSOFT_PHONE, 'count(*)');
        if ($fid > 0)
        {
            $select->where('factory_id = ?', $fid);   
            $view->assign('factory_id', $fid);
        }
        if ($machineip)
        {
        	$select->where('machineip like \'%'. $machineip . '%\'');   
        }
        
        if ($phone_num)
        {
        	$select->where('phone_num like \'%'. $phone_num . '%\'');   
        }
		...
		$select->order("p.phone_num");
        if ($fid > 0)
        {
            $select->where('f.factory_id = ?', $fid);   
        }
        if ($machineip)
        {
        	$select->where('machineip like \'%'. $machineip . '%\'');   
        ...
        //分页 
        $page_arr = array(
            'url_start' => $config['site_url'] . '/qnphone/list/fid/' . $fid . '/',
            'cur_page' => $page, 
