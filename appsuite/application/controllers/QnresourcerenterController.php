...
class QnresourcerenterController extends Zend_Controller_Action_Qn
{    
    ...
    public function addAction()
    {
    	...
    	$view->display('renter/resourceadd.tpl');
    }
	...
	public function adddoAction()
    {
		...
		if ($resource_name)
		{
            ...
            try
            {
				$db->insert($config['factory_enterprisecode'] . '_' . TABLE_QNSOFT_RESOURCE, $in_ary);
            } 
            catch(Exception $e){
        ...
        $this->msg("新增资源成功", "/qnresourcerenter/add");
    }
	...
	public function listAction()
	{
        ...
        $selectSum = $db->select();     
        $select = $db->select();
        $selectSum->from($config['factory_enterprisecode'] . '_' . TABLE_QNSOFT_RESOURCE, 'count(*)'); 
        $select->from($config['factory_enterprisecode'] . '_' . TABLE_QNSOFT_RESOURCE . ' as p', '*');
        ...
        //分页 
        $page_arr = array(
            'url_start' => $config['site_url'] . '/qnresourcerenter/list/',
            'cur_page' => $page, 
    	...
    	$view->display('renter/resourcelist.tpl');
	}
	...
	public function editAction()
	{
		...
		if (!$submit)
		{
			...
			$view->display('renter/resourceedit.tpl');
		}
		else
		{
		    $resource_name = $this->_getParam('resource_name');
			...
			$db->update($config['factory_enterprisecode'] . '_' . TABLE_QNSOFT_RESOURCE, $up_ary, $where);

			header('location: ' . $config['site_url'] . '/qnresourcerenter/list');
		}
	}
    ...
    {
    	$config = Zend_Registry::get('config');
    	header('location: ' . $config['site_url'] . '/qnfactoryrenter/list');
    }
