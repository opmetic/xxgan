...
class QnrolerenterController extends Zend_Controller_Action_Qn
{    
    ...
    public function addAction()
    {
    	...
    	$view->display('renter/roleadd.tpl');
    }
	...
	public function adddoAction()
    {
		...
		if ($role_name)
		{
			$in_ary = array(
                ...
                try
                {
				    $db->insert($config['factory_enterprisecode'] . '_' . TABLE_QNSOFT_ROLE, $in_ary);
                }  
                catch(Exception $e){
        ...
        $this->msg("新增角色成功", "/qnrolerenter/add");
    }
	...
	public function listAction()
	{
        ...
        $select = $db->select();
        $select->from($config['factory_enterprisecode'] . '_' . TABLE_QNSOFT_ROLE, 'count(*)');
        ...
        //分页 
        $page_arr = array(
            'url_start' => $config['site_url'] . '/qnrolerenter/list/',
            'cur_page' => $page, 
        ...
        $view->display('renter/rolelist.tpl');
	}
	...
	public function editAction()
	{
		...
		if (!$submit)
		{
			//角色信息
			$select = $db->select();
			$select->from($config['factory_enterprisecode'] . '_' . TABLE_QNSOFT_ROLE, '*');
			...
			$view->display('renter/roleedit.tpl');
		}
		else
		{
			$role_name = $this->_getParam('name');
            ...
            try
            {
                ...
                $db->update($config['factory_enterprisecode'] . '_' . TABLE_QNSOFT_ROLE, $up_ary, $where);    
            }  
            catch(Exception $e){
                $_POST['errmsg'] = '您输入有错误，请重新输入';
                $view->assign('post', $_POST);  
			...
			header('location: ' . $config['site_url'] . '/qnrolerenter/list');
		}
	}
