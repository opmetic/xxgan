...
class QnskillController extends Zend_Controller_Action_Qn
{    
	...
	public function listAction()
	{
        ...
        $select = $db->select();
        $select->from(TABLE_QNSOFT_SKILL, 'count(*)');
        $select->where('deleteflag = 0'); //未删除
        $sum = $db->fetchOne($select);
        
		$select = $db->select();
		$select->from(TABLE_QNSOFT_SKILL . ' as p', '*');
        ...
        //分页 
        $page_arr = array(
            'url_start' => $config['site_url'] . '/qnskill/list/',
            'cur_page' => $page, 
	...
	public function editAction()
	{
		...
		else
		{
            ...
            $urlArray = array(
                array('title' => "查看列表", 'url' => "/qnskill/list")
            );
