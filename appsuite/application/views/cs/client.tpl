...
$(function(){                
    ...
    $("#AgentAutoEnterIdle").click(function(){
        ...
        else
        {
        	...
        	$.ajax({
				type: "GET",
				url: "{/literal}{$system_config.img_url}{literal}/appsuite/checkmax",
				data: {skillcode: {/literal}{$skillcode}{literal}},
				dataType: "json",
...
function heartbeat()
{
	$.ajax({
		type: "GET",
		url: "{/literal}{$system_config.img_url}{literal}/appsuite/heartbeat",
		dataType: "json",
		success: function(msg){
		}
	}); 
...
-->
        ...
        <div style="height: 60px; border:#f00 0px solid;">
            <ul id="toolbar">
                ...
                <li style="width: 80px;">
                    ...
                    <input type="checkbox" id="AutoAnswer"{if $user.autoanswer == 1}checked="checked"{/if} disabled="true"><span style="color:#696969">自动应答</span>  <p/>
                </li>
                <li style="width: 100px;"> 
                    ...
                    {if $user.skillinternationflag == 1}
                    <a stype="" href="javascript:void(0);" class="easyui-splitbutton" plain="true" menu="#otherskill" style="margin-left:6px;">国际机票</a>
                    {/if}
                </li>
            </ul>
        </div>
        ...
        <div id="maintab" class="easyui-tabs" fit="true" border="false">
            <div title="起始页" style="padding:10px;overflow:hidden;"> 
                <div class="index_info_div">
                    ...
                    <li>平台：{$system_config.site_name}</li> 
                    <li>座席名：{$user.nickname}</li> 
					<li>座席工号： {$user.agentid}</li> 
					<li>座席分机号： {$user.DN}</li> 
                    </ul>
                </div>
	...
	{/literal}
	 ...
	 <!-- 目录 班长  -->
      ...
      <div id="cti_tool_pay">IVR支付</div>
      <div id="cti_tool_pay_manual">人工支付</div>
      <div id="cti_tool_pay_hotel">酒店支付</div>
      {if $userdefButtonex}
      <div class="menu-sep"></div>
      {foreach from=$userdefButtonex item=item name=listloop }
      	<div id="{$item.button_info}">{$item.button_name}</div> 
      {/foreach}
      {/if}
      </div>
      ...
      <span>全部</span>
            <div style="width:150px;">
                {foreach from=$userdefButton101 item=item name=listloop }
                  <div id="{$item.button_info}">{$item.button_name}</div> 
                {/foreach}
            </div>
      </div>
      <div class="menu-sep"></div>
      {/if}
      {foreach from=$userdefButton100 item=item name=listloop }
        <div id="{$item.button_info}">{$item.button_name}</div> 
      {/foreach}
      ...
      <div id="cti_tool_internation_cq">重庆 02155304965</div>
      <div id="cti_tool_internation_hn">湖南 02155304968</div>
      <div id="cti_tool_internation_sc">四川 02155304902</div>
      -->
...
{
{/literal} 
    var str = csAgentClient.manageData("<\?xml version=\"1.0\" \?><cmdbody><cmdtype>local</cmdtype><cmdname>setparam</cmdname><parameter><element name=\"workid\" type=\"\" value=\"{$user.workid}\" /><element name=\"agentid\" type=\"\" value=\"{$user.agentid}\" /><element name=\"username\" type=\"\" value=\"{$user.agentid}\" /><element name=\"photonum\" type=\"\" value=\"{$user.DN}\" /><element name=\"pwdmd5\" type=\"\" value=\"{$user.password}\" /><element name=\"mainip\" type=\"\" value=\"{$system_config.cti_server}\" /></parameter></cmdbody>");
    str = csAgentClient.manageData("<\?xml version=\"1.0\" \?><cmdbody><cmdtype>local</cmdtype><cmdname>setparam</cmdname><parameter><element name=\"skillcode\" type=\"\" value=\"{$skillcode}\" /><element name=\"vcid\" type=\"\" value=\"{$factoryInfo.factory_code}\" /></parameter></cmdbody>");
{literal}
}
catch(err)
{
