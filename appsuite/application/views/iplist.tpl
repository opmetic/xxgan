...
<tr>
	...
	<th>
		<select style="font-weight: bold; width: 100%;" id="selectbygroup">
			...
			{foreach from=$qnsoftip item=item key=key}
...
{foreach from=$iplist item=item name=iplistloop }
<tr align="center" id="{$item.id}">
	<td align="right">{$smarty.foreach.iplistloop.iteration}</td>
	<td>{if $item.ipaddr}{$item.ipaddr}{else}&nbsp;{/if}</td>
	<td>{if $item.ipgroup_txt}{$item.ipgroup_txt}{else}&nbsp;{/if}</td>
	<td>{if $item.type_txt}{$item.type_txt}{else}&nbsp;{/if}</td>
	<td>{if $item.addr}{$item.addr}{else}&nbsp;{/if}</td>
	<td>{if $item.pcname}{$item.pcname}{else}&nbsp;{/if}</td>
	<td>{if $item.des}{$item.des}{else}&nbsp;{/if}</td>
	<td>
