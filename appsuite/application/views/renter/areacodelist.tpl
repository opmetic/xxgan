...
function addDle()
{
	parent.updateWindows("新增区号", "{/literal}{$system_config.site_url}{literal}/qnareacoderenter/add/", true);
}

function editDle(title, id)
{
	parent.updateWindows("编辑 " + title, "{/literal}{$system_config.site_url}{literal}/qnareacoderenter/edit/id/" + id, true);
}
...
function confirmDle(id)
{
	...
	if (bln)
	{
		location.href = "{/literal}{$system_config.site_url}{literal}/qnareacoderenter/del/id/" + id;
	}
}
</script>
{/literal}
...
<table id="listtab" fit="true">
	...
	{foreach from=$list item=item name=listloop }
		<tr>
			...
			<td>{$item.areacode_province}</td>
			<td>{$item.areacode_city}</td>
            <td>{$item.areacode}</td> 
            <td>{$item.areacode_phonenum}</td> 
            <td>{$item.areacode_group}</td> 
            <td>{$item.areacode_class}</td> 
			<td>
				<a href="javascript:editDle('{$item.areacode_phonenum}', {$item.areacode_id})">编辑</a>
				<a href="javascript:confirmDle({$item.areacode_id})">删除</a>
			</td>
		</tr>
	{/foreach}
	</tbody>
