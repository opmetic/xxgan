...
<div style="background:#FFFFFF;padding:10px;">
    <form id="editscript" method="post" action="{$system_config.img_url}/qnbuttonexrenter/edit">
            ...
            <td>
                <select name="button_group" id="button_group">
                    {foreach from=$list item=item name=listloop }
                    	<option value="{$item.skill_code}" {if $buttonData.button_group == $item.skill_code}selected="selected"{/if}>{$item.skill_name}</option>
                    {/foreach}
                </select>
            <td><span style="color:#FF0000;">*</span></td>
        </tr>
