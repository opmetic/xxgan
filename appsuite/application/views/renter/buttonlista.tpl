...
function addDle()
{
	parent.updateWindows("新增按键", "{/literal}{$system_config.site_url}{literal}/qnbuttonrenter/add/", true);
}

function editDle(title, id)
{
	parent.updateWindows("编辑 " + title, "{/literal}{$system_config.site_url}{literal}/qnbuttonrenter/edit/id/" + id, true);
}
...
function confirmDle(id)
{
	...
	if (bln)
	{
		location.href = "{/literal}{$system_config.site_url}{literal}/qnbuttonrenter/del/id/" + id;
	}
}
</script>
{/literal}
...
<table id="listtab" fit="true">
	...
	{foreach from=$list item=item name=listloop }
		<tr>
			...
			<td>
                {if $item.button_group == 100}常用{/if}
                {if $item.button_group == 101}非常用{/if}
            </td>
			<td>
