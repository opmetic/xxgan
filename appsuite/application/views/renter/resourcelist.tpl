...
function updateList()
{
     location.href = "{/literal}{$system_config.img_url}{literal}/qnresourcerenter/list/sourcetype/" + $("#resource_type").val();
}
 
function viewDle(title, id)
{
	parent.updateWindows("查看 " + title, "{/literal}{$system_config.img_url}{literal}/qnresourcerenter/show/id/" + id, true);
}

function editDle(title, id)
{
	parent.updateWindows("编辑 " + title, "{/literal}{$system_config.img_url}{literal}/qnresourcerenter/edit/id/" + id, true);
}
...
function confirmDle(id)
{
	...
	if (bln)
	{
		location.href = "{/literal}{$system_config.img_url}{literal}/qnresourcerenter/del/id/" + id;
	}
}
</script>
{/literal}
