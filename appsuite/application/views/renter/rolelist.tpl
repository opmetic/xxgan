...
{
	parent.updateWindows("权限 " + title, "{/literal}{$system_config.img_url}{literal}/qnauthrenter/edit/id/" + id, true);
}

function editDle(title, id)
{
	parent.updateWindows("编辑 " + title, "{/literal}{$system_config.img_url}{literal}/qnrolerenter/edit/id/" + id, true);
}
...
function confirmDle(id)
{
	...
	if (bln)
	{
		location.href = "{/literal}{$system_config.img_url}{literal}/qnrolerenter/del/id/" + id;
	}
}
</script>
{/literal}
...
<table id="listtab" fit="true">
	...
	{foreach from=$list item=item name=listloop }
		<tr>
			...
			<td>{$system_config.factory_name}</td>
            <td>{$item.role_dsc}</td> 
			<td>
