...
function addDle(id)
{
	parent.updateWindows("新增脚本", "{/literal}{$system_config.site_url}{literal}/scriptrenter/add/id/" + id, true);
}

function editDle(title, id)
{
	parent.updateWindows("编辑 " + title, "{/literal}{$system_config.site_url}{literal}/scriptrenter/edit/id/" + id, true);
}
...
function confirmDle(id)
{
	...
	if (bln)
	{
		location.href = "{/literal}{$system_config.site_url}{literal}/scriptrenter/del/id/" + id;
	}
}
</script>
{/literal}
...
<div style="background:#FFFFFF;padding:10px;">
	...
	<form id="editscript" method="post" action="{$system_config.site_url}/scriptrenter/list">
	<table>
		<tr>
		<td width="60">
		用户：
		...
		<td width="200">
            ...
            <option value="{$item.uid}" {if $id == $item.uid}selected{/if}>{$item.nickname}[工号:{$item.workid}] </option>
            {/foreach}
			</select>
		</td>
		<td width="60">
