...
<div style="background:#FFFFFF;padding:10px;">
    <form id="editscript" method="post" action="{$system_config.img_url}/qnresource/edit">
    <input type="hidden" name="id" value="{$resourceData.resource_id}" />
    <div style="color: #FF0000;">{$post.errmsg}</div>
	<table cellpadding="10" cellspacing="10">
		<tr>
			<td align="right" width="100" >资源名:</td>
			<td><input class="easyui-validatebox" type="text" name="resource_name" id="resource_name" required="true" width="200" value="{$resourceData.resource_name}"></input></td>
		...
		<tr>
            ...
            <td>
                <select id="factoryid" name="factoryid">
                    {foreach from=$factoryData item=item name=listloop }
                    <option value="{$item.factory_id}" {if $resourceData.factory_id == $item.factory_id}selected{/if}>{$item.factory_name}</option>
                    {/foreach}
                </select>
            </td>
            <td></td>
        ...
        <tr>
            ...
            <td>
                <select id="resource_type" name="resource_type">
                    ...
                    <option value="{$key2}" {if $resourceData.resource_type == $item2}selected{/if}>{$item2}</option>
                    {/foreach}
                </select>
            </td>
            <td></td>
        ...
        <tr>
            ...
            <td><input type="text" name="resource_dsc" id="resource_dsc" required="true" width="500" value="{$resourceData.resource_dsc}"></input></td>
            <td></td>
        </tr>
		<tr>
			<td align="right"> </td>
