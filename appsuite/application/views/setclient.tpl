...
<script>
	 $(function(){
		$("input[name='AutoSignin']").click(function(){
			$.get("{/literal}{$system_config.site_url}/appsuite/setclient/id/{$user.uid}{literal}/op/autosignin/value/" + $(this).val());
		//	AppSuiteObject("command", "autosignin", $(this).val());
		});
		$("input[name='InitialReady']").click(function(){
			$.get("{/literal}{$system_config.site_url}/appsuite/setclient/id/{$user.uid}{literal}/op/initialready/value/" + $(this).val());
		//	AppSuiteObject("command", "initialready", $(this).val());
		});		
		$("input[name='AutoAnswer']").click(function(){
			$.get("{/literal}{$system_config.site_url}/appsuite/setclient/id/{$user.uid}{literal}/op/autoanswer/value/" + $(this).val()); 
			AppSuiteObject("command", "autoanswer", $(this).val());
		});
		$("input[name='AgentAutoEnterIdle']").click(function(){
			$.get("{/literal}{$system_config.site_url}/appsuite/setclient/id/{$user.uid}{literal}/op/agentautoenteridle/value/" + $(this).val()); 
...
<div class="index_info_div">
	<table class="form-table">
		<tr>
			...
			<td width=120>
				<input name="AutoSignin" type="radio" value="yes" {if $user.autosignin == 1}checked{/if}/>自动签入<br/>
				<input name="AutoSignin" type="radio" value="no" {if $user.autosignin == 0}checked{/if}/>不自动签入
			</td>
		...
		<tr>
			...
			<td width=120>
				<input name="InitialReady" type="radio" value="yes" {if $user.initialready == 1}checked{/if}/>签入后自动示闲<br/>
				<input name="InitialReady" type="radio" value="no" {if $user.initialready == 0}checked{/if}/>不签入后自动示闲
			</td>
		...
		<tr>
			...
			<td>
				<input name="AutoAnswer" type="radio" value="yes" {if $user.autoanswer == 1}checked{/if}/>自动应答<br/>
				<input name="AutoAnswer" type="radio" value="no" {if $user.autoanswer == 0}checked{/if} />不自动应答
			</td>
		...
		<tr>
			...
			<td>
				<input name="AgentAutoEnterIdle" type="radio" value="yes" {if $user.agentautoenteridle == 1}checked{/if}/>自动示闲<br/>
				<input name="AgentAutoEnterIdle" type="radio" value="no" {if $user.agentautoenteridle == 0}checked{/if}/>不自动示闲
			</td>
