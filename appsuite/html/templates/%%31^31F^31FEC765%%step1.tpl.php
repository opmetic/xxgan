<?php /* Smarty version 2.6.20, created on 2011-03-20 21:59:02
...
<script type="text/javascript">
$(function(){
    setTimeout("undateModel()", 1000);
    
    $("#localIP").change(function(){
        undateModel();
    });
});

function undateModel()
{ 
    $.ajax({
        type: "GET",
        url: "'; ?>
<?php echo $this->_tpl_vars['system_config']['img_url']; ?>
<?php echo '/qnphone/getdn",
        dataType: "json", /**/
        data: {ip: $("#localIP").val()},
        success: function(msg){    
            $("#model").val(msg.basemodel);
        }
    }); 
}
</script>
...
/instal/step2">
	...
	<table class="form-table">
		<tr>
			...
			<td><input name="model" id="model" type="text" size="25" value="" /></td>
			<td>座席机器在CTI服务器上的唯一标识。请保持默认值，除非您知道如何修改。</td>
		</tr>
		<tr>
			<th scope="row"><label for="ctiip">CTI服务地址</label></th>
			<td><input name="ctiip" id="ctiip" type="text" size="25" value="<?php echo $this->_tpl_vars['system_config']['cti_server']; ?>
"/></td>
			<td>CTI服务器上IP地址。请保持默认值，除非您知道如何修改。</td>
		</tr>
		<tr>
			<th scope="row"><label for="ctiport">CTI服务端口</label></th>
			<td><input name="ctiport" id="ctiport" type="text" size="25" value="<?php echo $this->_tpl_vars['system_config']['cti_server_port']; ?>
"/></td>
			<td>CTI服务器端口。请保持默认值，除非您知道如何修改。</td>
		</tr>
		<tr>
			<th scope="row"><label for="proxynum">PROXY NUM</label></th>
			<td><input name="proxynum" id="proxynum" type="text" size="25" value="<?php echo $this->_tpl_vars['system_config']['proxy_num']; ?>
"/></td>
			<td>PROXY NUM。请保持默认值，除非您知道如何修改。</td>
		</tr>
		<tr>
			<th scope="row"><label for="acdip">ACD服务地址</label></th>
			<td><input name="acdip" id="acdip" type="text" size="25" value="<?php echo $this->_tpl_vars['system_config']['acd_server']; ?>
"/></td>
			<td>ACD服务器上IP地址。请保持默认值，除非您知道如何修改。</td>
		</tr>
		<tr>
			<th scope="row"><label for="acdport">ACD服务端口</label></th>
			<td><input name="acdport" id="acdport" type="text" size="25" value="<?php echo $this->_tpl_vars['system_config']['acd_server_port']; ?>
"/></td>
			<td>ACD服务器端口。请保持默认值，除非您知道如何修改。</td>
		</tr>
		<tr>
			<th scope="row"><label for="areacord">区号</label></th>
			<td><input name="areacord" id="areacord" type="text" size="25" value="571"/></td>
